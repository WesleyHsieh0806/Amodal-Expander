...
from gtr.data.tao_amodal_dataset_mapper import TAOAmodalDatasetMapper
from gtr.data.tao_amodal_dataset_modal_match_mapper import TAOAmodalDatasetModalMatchMapper
...
def do_test(cfg, model):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        if evaluator_type == "lvis":
            # Might need to implement another Evaluator for TAO-Amodal
        ...
        if not cfg.VIDEO_INPUT:
            ...
            '''
            *   In inference_on_dataset() function,
            *   the following process happened: 
            *   1. model(input) = output
            *   2. evaluator.process(input, output)
            *   3. evaluator.evaluate()
            '''
            results[dataset_name] = inference_on_dataset(
                model, data_loader, evaluator, 
            )  # return the result of evaluator.evaluate()
...
def do_train(cfg, model, resume=False):
    ...
    DatasetMapperClass = GTRDatasetMapper if cfg.VIDEO_INPUT else \
        CustomDatasetMapper
    if (cfg.VIDEO_INPUT and cfg.INPUT.USE_MODAL_MATCH):
        DatasetMapperClass = TAOAmodalDatasetModalMatchMapper
    elif (cfg.VIDEO_INPUT and cfg.INPUT.USE_PASTE_AND_OCCLUDE):
        DatasetMapperClass = TAOAmodalDatasetMapper
...
def setup(args):
    ...
    setup_logger(output=cfg.OUTPUT_DIR, \
        distributed_rank=comm.get_rank(), name="gtr")
    return cfg
...
if __name__ == "__main__":
    # https://detectron2.readthedocs.io/en/latest/_modules/detectron2/engine/defaults.html
