...
def _freeze_except_cascade_reg(model):
    for v in model.parameters():
        v.requires_grad = False
    ...
    try:
        ...
        for child in model.roi_heads.box_predictor.children():
            for v in child.bbox_pred.parameters():
                v.requires_grad = True
...
def _freeze_except_proposal_and_cascade_reg(model):
    for v in model.parameters():
        v.requires_grad = False
    ...
    try:
        for v in model.module.proposal_generator.parameters():
            v.requires_grad = True
    except:
        for child in model.proposal_generator.parameters():
            v.requires_grad = True
...
def _freeze_except_amodal_expander(model):
    for v in model.parameters():
        v.requires_grad = False
    
    try:
        for child in model.module.roi_heads.amodal_expander.children():
            for v in child.parameters():
                v.requires_grad = True
    except:
        for child in model.roi_heads.amodal_expander.children():
            for v in child.parameters():
                v.requires_grad = True
...
def check_if_freeze_model(model, cfg):
    ...
    elif cfg.MODEL.FREEZE_TYPE == 'ExceptRegression':
        print('Freezing  except cascade regression layers!')
        model = _freeze_except_cascade_reg(model)
    elif cfg.MODEL.FREEZE_TYPE == 'ExceptProposalNRegression':
        print('Freezing  except proposal generator and cascade regression layers!')
        model = _freeze_except_proposal_and_cascade_reg(model)
    ...
    elif cfg.MODEL.FREEZE_TYPE == 'ExceptAmodalExpander':
        logger.info('Freezing except Amodal Expander')
        model = _freeze_except_amodal_expander(model)
    else:
