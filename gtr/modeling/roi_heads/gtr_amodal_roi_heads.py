...
from .gtr_roi_heads import GTRROIHeads
from .amodal_expander import AmodalExpander

logger = logging.getLogger(__name__)
@ROI_HEADS_REGISTRY.register()
class GTRAmodalROIHeads(GTRROIHeads):
    @configurable
    def __init__(self, **kwargs):
        ...
        amodal_expander = kwargs.pop('amodal_expander', None)
        self.expand_last_stage_only = kwargs.pop('expand_last_stage_only', None)
        self.use_modal_match = cfg.INPUT.USE_MODAL_MATCH
        ...
        self.amodal_expander = nn.ModuleList(amodal_expander)
    ...
    def _forward_transformer(self, proposals, reid_features, query_frame=None, only_get_temporal=False):
        T = len(proposals)
        n_t = [len(x) for x in proposals]
    ...
    def _get_boxes_time(self, instances):
        ...
        for t, p in enumerate(instances):
            ...
            elif p.has('match_boxes') and self.use_modal_match:
                p_boxes = p.match_boxes.tensor.clone()
            else:
    ...
    def _init_box_head(self, cfg, input_shape):
        ...
        for box_head, bbox_reg_weights in zip(ret['box_heads'], cascade_bbox_reg_weights):
            ...
            # We can choose whether or not to share weights between different stages.
            if cfg.MODEL.AMODAL_EXPANDER.CASCADE_SHARE_WEIGHTS and amodal_expander:
                amodal_expander.append(amodal_expander[0])
            else:
                amodal_expander.append(AmodalExpander(
                    box_head.output_shape, cfg.MODEL.AMODAL_EXPANDER.HIDDEN_DIM, len(cascade_bbox_reg_weights[0]), cfg.MODEL.AMODAL_EXPANDER.NUM_LAYER, 
                    dropout=cfg.MODEL.AMODAL_EXPANDER.DROPOUT, zero_init=cfg.MODEL.AMODAL_EXPANDER.ZERO_INIT, use_temporal=cfg.MODEL.AMODAL_EXPANDER.USE_TEMPORAL,
                    use_proposal_feature=cfg.MODEL.AMODAL_EXPANDER.USE_PROPOSAL_FEATURE, use_modal_delta=cfg.MODEL.AMODAL_EXPANDER.USE_MODAL_DELTA
                ))
        ret['box_predictors'] = box_predictors
        ret['amodal_expander'] = amodal_expander
        ret['expand_last_stage_only'] = cfg.MODEL.AMODAL_EXPANDER.ONLY_LAST_STAGE
        self.is_temporal_amodal_expander = cfg.MODEL.AMODAL_EXPANDER.USE_TEMPORAL
        self.use_temporal_from_tracker = cfg.MODEL.AMODAL_EXPANDER.USE_TEMPORAL_FROM_TRACKER
        if self.is_temporal_amodal_expander:
            logger.warning('Using temporal amodal expander. Please ensure the input images are from the same sequence and ordered.')
        return ret

    def _get_temporal_feature_from_tracker(self, features, proposals):
        features = [features[f] for f in self.asso_in_features]
        proposal_boxes = [x.proposal_boxes for x in proposals] # 
        pool_features = self.asso_pooler(features, proposal_boxes)
        reid_features = self.asso_head(pool_features)
        reid_features = reid_features.view(
            1, -1, self.feature_dim) # 1 x N x F, N denotes total # boxes
        n_t = [len(x) for x in proposals]

        # For the covenience of implementation, we query from all frames instead of only the last frame.
        # However, we don't modify the boxes in the past frames to align with the setting of online trackers.
        feats = self._forward_transformer(proposals, reid_features, only_get_temporal=True)  # L x [1 x N x F], L denotes # output layers in the transformer  decoder
        feats = feats.reshape([-1, feats.shape[-1]])  # (N, F)
        return feats

    def _expand_modal_box(self, modal_delta, features, proposals, stage, pyramid_features):
        """
        modal_delta: (N, 4) tensor, where N denotes total # boxes
        features:
            a list of feature map extracted by backbone+FPN
            each feature map is shaped as (B, C, H, W), where B is the batch size
        proposals:
            A list of B instances object with the following field:
                proposal boxes
                objectness logits
        stage:
            which stage we are at
        """
        box_features = self.box_pooler(features, [x.proposal_boxes for x in proposals])  # (N, C, H, W)

        if self.training:
            box_features = _ScaleGradient.apply(box_features, 1.0 / self.num_cascade_stages)
        box_features = self.box_head[stage](box_features)  # (N, F)

        if self.is_temporal_amodal_expander:
            if self.use_temporal_from_tracker:
                # Get temporal features from transformer
                temp_features = self._get_temporal_feature_from_tracker(pyramid_features, proposals)  # (N, F)
                box_features = torch.cat([box_features, temp_features], dim=-1)
            else:
                # Get temporal features from proposal features using cross attention layers
                box_features = self.amodal_expander[stage].get_temporal_feature(box_features)
            
        return self.amodal_expander[stage](modal_delta, proposals, box_features)
    ...
    def _match_and_label_boxes(self, proposals, stage, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            if self.use_modal_match:
                # Use modal boxes for matching
                match_quality_matrix = pairwise_iou(
                    targets_per_image.match_boxes, proposals_per_image.proposal_boxes
                )
            else:
                match_quality_matrix = pairwise_iou(
    ...
    def _forward_box(self, pyramid_features, proposals, targets=None):
        """
        ...
        features = [pyramid_features[f] for f in self.box_in_features]
        ...
        for k in range(self.num_cascade_stages):
            ...
            if not(self.expand_last_stage_only and k != (self.num_cascade_stages - 1)):
                amodal_delta = self._expand_modal_box(predictions[1], features, proposals, k, pyramid_features)
                predictions = predictions[0], amodal_delta
