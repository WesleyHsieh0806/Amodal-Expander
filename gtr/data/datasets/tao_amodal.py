...
__all__ = ["load_tao_amodal_json", "register_tao_amodal_instances", "get_tao_amodal_instances_meta"]


def register_tao_amodal_instances(name, metadata, json_file, image_root):
    """
    Register a dataset in tao_amodal's json annotation format for instance detection and segmentation.
    ...
    """
    DatasetCatalog.register(name, lambda: load_tao_amodal_json(json_file, image_root, name))
    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, evaluator_type="tao_amodal", **metadata
    )

def register_tao_amodal_v1_instances(name, metadata, json_file, image_root):
    """
    Register a dataset in tao_amodal's json annotation format for instance detection and segmentation using LVIS v1 evaluator
...
def load_tao_amodal_json(json_file, image_root, dataset_name=None):
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            # In Tao-Amodal dataset, we use amodal bbox
            obj = {"bbox": anno["amodal_bbox"], "bbox_mode": BoxMode.XYWH_ABS}
            if id_map:
...
def get_tao_amodal_instances_meta():
...
def get_tao_amodal_v1_instances_meta():
...
_PREDEFINED_SPLITS_TAO_Amodal = {
    "tao_amodal_train": ("tao/frames/", "tao/amodal_annotations/train_with_freeform_amodal_boxes_may12_2022.json"),
    "tao_amodal_val": ("tao/frames/", "tao/amodal_annotations/validation_with_freeform_amodal_boxes_Aug10_2022.json"),
    "tao_amodal_test": ("tao/frames/", "tao/amodal_annotations/test_with_freeform_amodal_boxes_Jan10_2023.json"),
}

for key, (image_root, json_file) in _PREDEFINED_SPLITS_TAO_Amodal.items():
    register_tao_amodal_instances(
        key,
        get_tao_amodal_instances_meta(),
...
for key, (image_root, json_file) in _PREDEFINED_SPLITS_TAO_AmodalV1.items():
    register_tao_amodal_v1_instances(
        key,
