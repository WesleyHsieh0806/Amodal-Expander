...
def get_video_dataset_dicts(
    ...
    for source_id, (dataset_name, dicts) in enumerate(
        ...
        # Generate pseudo annotations for a video to support RepeatFactorSampler
        for video_id in videos:
            if 'annotations' in videos[video_id]:
                raise ValueError("We are creating pseudo annotations for each video to support RepeatFactorSampler. "
                                "This operation could modify the original annotations if they exist. Remove the next line to avoid bugs.")
            videos[video_id]['annotations'] = [ann for img in videos[video_id]['images'] for ann in img['annotations'] ]
